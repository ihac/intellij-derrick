...
import xyz.ihac.intellij.plugin.derrick.{
  DerrickOptionProvider,
  DerrickProjectOptionProvider
}
...
class InitAction extends AnAction with ActionHelper {
  ...
  override def actionPerformed(e: AnActionEvent): Unit = {
    ...
    val eventLog =
      ToolWindowManager.getInstance(project).getToolWindow("Event Log")
    if (!eventLog.isVisible)
      eventLog.show(null)

    val option = ServiceManager.getService(classOf[DerrickOptionProvider])
    val projOption =
      ServiceManager.getService(project, classOf[DerrickProjectOptionProvider])
    ...
    new BackgroundTask(() => {
      ...
      /**
        ...
        */
      val paramsTry = Try {
        Derrick.get_riggings_and_params(option.getDerrickExecPath,
                                        projOption.getWorkDir)
      }
      paramsTry match {
        case Success(resp) =>
          if (resp.status == "success")
            ...
            failFrom(
              s"<derrick init> failed with an unrecognized error. ${resp.code} - ${resp.message}")
        case Failure(e) =>
          failFrom(
            s"unexpected error in getting riggings and params from derrick: ${e.getMessage}")
      }
      if (!isRunning) return
      ...
      new WaitableUITask(() => {
        ...
        val configDialog =
          new DerrickConfigForm(project, action, resp.riggingsAndParamsInJava)
        configDialog.show()
      ...
      val initTry = Try {
        val derrick =
          new Derrick(option.getDerrickExecPath, projOption.getWorkDir)
      ...
      initTry match {
        case Success(resp) =>
          if (resp.status == "success") done()
            ...
            failFrom(
              s"<derrick init> failed in generating Dockerfile and other template files: ${resp.code} - ${resp.message}")
        case Failure(e) =>
          failFrom(
            s"unexpected error in invoking derrick init with user-defined parameters: ${e.getMessage}")
      }
    }).run
  }
}
